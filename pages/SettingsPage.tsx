  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto w-full space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" readOnly value={username} className="w-full pl-10 pr-4 py-2.5 bg-gray-100 border border-gray-300 rounded-xl text-gray-600 font-medium sm:text-sm shadow-inner cursor-not-allowed" />
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden h-fit">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-500 mb-5 leading-relaxed">{t(language, 'backupRestoreDesc')}</p>
              <div className="flex flex-col sm:flex-row gap-3">
                <button onClick={handleBackupData} className="flex-1 flex justify-center items-center px-4 py-3 bg-indigo-50 text-indigo-700 rounded-xl text-sm font-bold hover:bg-indigo-100 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 border border-indigo-200">
                  <DownloadCloud className="w-4 h-4 mr-2" />
                  {t(language, 'backupData')}
                </button>
                <input type="file" accept=".json" ref={fileInputRef} onChange={handleFileChange} className="hidden" />
                <button onClick={triggerFileInput} className="flex-1 flex justify-center items-center px-4 py-3 bg-white text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-300 border border-gray-300">
            ...
            <form onSubmit={handlePasswordChange} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" required value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm transition-shadow shadow-sm" />
                </div>
              </div>
              <div className="border-t border-gray-100 pt-5">
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t(language, 'newPassword')}</label>
                <input type="password" required value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm transition-shadow shadow-sm mb-4" />
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t(language, 'confirmPassword')}</label>
                <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm transition-shadow shadow-sm" />
              </div>
              <button type="submit" className="w-full flex justify-center items-center px-4 py-3 bg-brand-600 text-white rounded-xl text-sm font-bold hover:bg-brand-700 transition-all shadow-sm active:scale-95 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 mt-2">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl shadow-sm overflow-hidden mt-8 transition-all duration-300">
        <button onClick={() => setIsLogsExpanded(!isLogsExpanded)} className="w-full px-5 py-4 flex items-center justify-between hover:bg-slate-100/80 transition-colors focus:outline-none">
        ...
        {isLogsExpanded && (
          <div className="border-t border-slate-200 px-5 pb-5 pt-4">
            <div className="flex flex-col sm:flex-row justify-end items-center mb-4 space-y-3 sm:space-y-0 sm:space-x-3">
              <button onClick={handleExportLogs} disabled={activityLogs.length === 0} className="w-full sm:w-auto flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                <Download className="w-4 h-4 mr-2" />
                {t(language, 'exportExcel')}
              </button>
              <button onClick={requestClearLogs} disabled={activityLogs.length === 0} className="w-full sm:w-auto flex items-center justify-center px-4 py-2 bg-red-50 border border-red-200 rounded-xl text-sm font-medium text-red-600 hover:bg-red-100 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  {recentLogs.length === 0 ? (
                    ...
                    recentLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-5 py-3 whitespace-nowrap text-slate-500 font-medium">{formatDateTime(log.timestamp)}</td>
                        <td className="px-5 py-3 whitespace-nowrap font-bold text-slate-700">{log.action}</td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all text-center">
            ...
            <div className="flex space-x-3">
              <button onClick={() => { setConfirmModal(null); setPendingRestoreData(null); }} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 font-medium transition-colors">
                {t(language, 'cancel')}
              </button>
              <button onClick={() => { confirmModal.onConfirm(); setConfirmModal(null); }} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 font-bold shadow-sm transition-all">
      ...
      {alert && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all text-center">
            {alert.type === 'success' ? <CheckCircle2 className="w-12 h-12 text-green-500 mx-auto mb-3" /> : <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-3" />}
            ...
            <button onClick={() => setAlert(null)} className={`w-full px-4 py-2.5 text-white rounded-xl font-bold shadow-sm transition-all active:scale-95 ${alert.type === 'success' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}>
              OK
            </button>
          </div>
        </div>
